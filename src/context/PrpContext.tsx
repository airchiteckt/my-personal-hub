...
interface PrpContextType {
  ...
  saveJournalEntry: (date: string, content: string, mood?: string, energyLevel?: number, energyMorning?: number, energyAfternoon?: number, energyEvening?: number, lunarData?: any) => Promise<void>;
...
export function PrpProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (jRes.data) setJournalEntries(jRes.data.map((r: any) => ({ id: r.id, entryDate: r.entry_date, content: r.content, mood: r.mood, energyLevel: r.energy_level, energyMorning: r.energy_morning, energyAfternoon: r.energy_afternoon, energyEvening: r.energy_evening, lunarData: r.lunar_data, createdAt: r.created_at, updatedAt: r.updated_at })));
    ...
    const channels = [
      ...
      supabase.channel('journal-changes').on('postgres_changes', { event: '*', schema: 'public', table: 'journal_entries', filter: `user_id=eq.${userId}` }, () => {
        supabase.from('journal_entries').select('*').eq('user_id', userId).order('entry_date', { ascending: false }).then(({ data }) => {
          if (data) setJournalEntries(data.map((r: any) => ({ id: r.id, entryDate: r.entry_date, content: r.content, mood: r.mood, energyLevel: r.energy_level, energyMorning: r.energy_morning, energyAfternoon: r.energy_afternoon, energyEvening: r.energy_evening, lunarData: r.lunar_data, createdAt: r.created_at, updatedAt: r.updated_at })));
        });
      }).subscribe(),
  ...
  const saveJournalEntry = useCallback(async (date: string, content: string, mood?: string, energyLevel?: number, energyMorning?: number, energyAfternoon?: number, energyEvening?: number, lunarData?: any) => {
    if (!userId) return;
    ...
    const dbPayload: any = {
      ...
      energy_evening: energyEvening ?? null,
      lunar_data: lunarData ?? null,
    };
    if (existing) {
      setJournalEntries(prev => prev.map(j => j.id === existing.id ? { ...j, content, mood, energyLevel, energyMorning, energyAfternoon, energyEvening, lunarData } : j));
      await supabase.from('journal_entries').update(dbPayload).eq('id', existing.id);
    } else {
      const { data, error } = await supabase.from('journal_entries').insert({
        user_id: userId, entry_date: date, ...dbPayload,
      }).select().single();
      if (!error && data) {
        setJournalEntries(prev => [{ id: data.id, entryDate: data.entry_date, content: data.content, mood: data.mood, energyLevel: data.energy_level, energyMorning: data.energy_morning, energyAfternoon: data.energy_afternoon, energyEvening: data.energy_evening, lunarData: data.lunar_data as any, createdAt: data.created_at, updatedAt: data.updated_at }, ...prev]);
      }
    }
