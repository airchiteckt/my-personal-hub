...
import { RotateCcw, Gauge, Clock, Target, FolderKanban } from 'lucide-react';
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-2">
            ...
            Priority Control Cabin
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Configura come il sistema calcola la priorità effettiva
          </p>
        </div>
      ...
      <div className="space-y-4">
            ...
            <div className="space-y-4 pt-2 border-t">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                <div className="space-y-2">
                  ...
                  <Slider
                    value={[prioritySettings.deadlineCriticalHours]}
                    onValueChange={([v]) => update('deadlineCriticalHours', v)}
                    min={6} max={48} step={6}
                  />
                  <Label className="text-xs text-muted-foreground">
                    Boost: +{prioritySettings.deadlineCriticalBoost}
                  </Label>
                  <Slider
                    value={[prioritySettings.deadlineCriticalBoost]}
                    onValueChange={([v]) => update('deadlineCriticalBoost', v)}
                    min={1} max={5} step={1}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Slider
                    value={[prioritySettings.deadlineHighHours]}
                    onValueChange={([v]) => update('deadlineHighHours', v)}
                    min={12} max={96} step={12}
                  />
                  <Label className="text-xs text-muted-foreground">
                    Boost: +{prioritySettings.deadlineHighBoost}
                  </Label>
                  <Slider
                    value={[prioritySettings.deadlineHighBoost]}
                    onValueChange={([v]) => update('deadlineHighBoost', v)}
                    min={1} max={5} step={1}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Slider
                    value={[prioritySettings.deadlineAttentionHours]}
                    onValueChange={([v]) => update('deadlineAttentionHours', v)}
                    min={24} max={168} step={24}
                  />
                  <Label className="text-xs text-muted-foreground">
                    Boost: +{prioritySettings.deadlineAttentionBoost}
                  </Label>
                  <Slider
                    value={[prioritySettings.deadlineAttentionBoost]}
                    onValueChange={([v]) => update('deadlineAttentionBoost', v)}
                    min={1} max={5} step={1}
                  />
                </div>
              </div>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Switch
              checked={prioritySettings.strategicWeightEnabled}
              onCheckedChange={v => update('strategicWeightEnabled', v)}
            />
          </div>
              ...
              {[
                ...
                <div key={item.key} className="flex items-center gap-4">
                  ...
                  <span className="text-sm font-mono w-8 text-right">
                    {prioritySettings[item.key] > 0 ? '+' : ''}{prioritySettings[item.key]}
                  </span>
                </div>
              ))}
            </div>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Switch
              checked={prioritySettings.impactEffortEnabled}
              onCheckedChange={v => update('impactEffortEnabled', v)}
            />
          </div>
          ...
          {prioritySettings.impactEffortEnabled && (
            <div className="space-y-3 pt-2 border-t mt-4">
              <div className="flex items-center gap-4">
                ...
                <Slider
                  value={[prioritySettings.impactMultiplier]}
                  onValueChange={([v]) => update('impactMultiplier', v)}
                  min={1} max={5} step={1}
                  className="flex-1"
                />
              ...
              <div className="flex items-center gap-4">
                ...
                <Slider
                  value={[prioritySettings.effortPenalty]}
                  onValueChange={([v]) => update('effortPenalty', v)}
                  min={0} max={3} step={1}
                  className="flex-1"
                />
