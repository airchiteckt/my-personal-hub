...
async function syncConnection(admin: any, conn: any) {
  ...
  // Preserve user prefs (enabled, color, enterprise_id)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      try { ({ connection_id } = await req.json()); } catch { /* no body */ }
    }

    const admin = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);

    let query = admin.from("google_calendar_connections").select("*").eq("user_id", user.id);
    if (connection_id) query = query.eq("id", connection_id);

    const { data: conns, error } = await query;
    if (error) throw error;
    if (!conns || conns.length === 0) {
      return new Response(JSON.stringify({ error: "No connections" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const results: any[] = [];
    let totalEvents = 0, totalCals = 0;
    for (const c of conns) {
      try {
        const r = await syncConnection(admin, c);
        results.push({ connection_id: c.id, email: c.google_email, ...r });
        totalEvents += r.events;
        totalCals += r.calendars;
      } catch (e) {
    ...
    return new Response(JSON.stringify({ ok: true, events: totalEvents, calendars: totalCals, results }), {
