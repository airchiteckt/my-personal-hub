...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = buildRadarChatPrompt({ ctx, now, channel: "telegram" });
      ...
      if (INSTANT.has(name)) {
        ...
        if (res.error) {
          ...
          try {
            ...
            human = cleanReply((j?.choices?.[0]?.message?.content ?? "").trim());
          } catch { /* ignore */ }
          await send(chatId, human || `Mi manca un dato per completare: ${res.error}`);
        } else {
